...
var UserTypePropToSql = map[string]string{ 
	"ID":`rgt.ut_id`,
	"Name":`rgt.name`,
}
...
var UserPropToSql = map[string]string{ 
	"ID":`regu.u_id`,
	"Updated":`regu.updated`,
	"Name":`regu.name`,
	"FirstName":`regu.firstname`,
	"UserType.ID":`regu.registration_user_type_ut_id`,
	"UserType.Name":`regu.registration_user_type_name`,
}



func UserFrom_lvl2() string { 
	return `FROM registration_user regu`
}

func UserSelect_lvl2() string { 
...
func LoadUser_lvl2(rs *sql.Rows) (*User, error) {
	result, err := LoadUsers_lvl2(rs)
	if err != nil {
		return nil, err
	}
	if len(result) > 1 {
...
func LoadUsers_lvl2(rs *sql.Rows) ([]*User, error) {
	var err error
	defer rs.Close()	
	result := make([]*User, 0)
